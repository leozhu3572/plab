...
import { Case, Material, Message, Role, Thread } from "../types";
import { estimateTokens, fileToBase64, generateId } from "../utils";
...
const CaseEditor: React.FC<CaseEditorProps> = ({
  ...
  const [initialThreads, setInitialThreads] = useState<string[]>(
    initialCase ? [] : [""]
  );
  ...
  const handleSave = async () => {
    ...
    if (!initialCase && initialThreads.length > 0) {
      const validArguments = initialThreads.filter(
      ...
      for (let i = 0; i < validArguments.length; i++) {
        ...
        setLoadingMessage(
          `Analyzing Argument ${i + 1} of ${validArguments.length}...`
        );
        ...
        try {
          ...
          const errorMsg: Message = {
            ...
            role: Role.MODEL,
            content:
              "[System] AI analysis failed temporarily. You can regenerate this response in the dashboard.",
            timestamp: Date.now(),
          };
    ...
    try {
      ...
      if (sizeBytes > 1000000) {
        alert(
          "Case data is too large for the database. Please remove some images or reduce content."
        );
        ...
        {!initialCase && (
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              Initial Arguments (Threads)
            </label>
            <p className="text-xs text-slate-500 mb-3">
              Enter your arguments. The AI Opposing Counsel will analyze and
              respond to each one upon creation.
            </p>
